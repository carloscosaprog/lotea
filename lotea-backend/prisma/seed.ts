...
async function main() {
  ...
  const tecnologia = await createCategoria(
    "Tecnología y electrónica",
    "tecnologia-electronica",
    "smartphone",
  );

  const moda = await createCategoria(
    "Moda y accesorios",
    "moda-accesorios",
    "shirt",
  );
  ...
  const electrodomesticos = await createCategoria(
    "Electrodomésticos",
    "electrodomesticos",
    "washing-machine",
  );

  const deporte = await createCategoria(
    "Deporte y ocio",
    "deporte-ocio",
    "dumbbell",
  );

  const gaming = await createCategoria(
    "Videojuegos y gaming",
    "videojuegos-gaming",
    "gamepad-2",
  );

  const cine = await createCategoria(
  ...
  const coleccionismo = await createCategoria(
    "Coleccionismo",
    "coleccionismo",
    "package",
  );

  const bricolaje = await createCategoria(
    "Herramientas y bricolaje",
    "herramientas-bricolaje",
    "hammer",
  );

  const industria = await createCategoria(
    "Industria y negocio",
    "industria-negocio",
    "briefcase",
  );

  const motor = await createCategoria(
    "Motor y accesorios",
    "motor-accesorios",
    "car",
  );

  const salud = await createCategoria(
    "Salud y belleza",
    "salud-belleza",
    "heart-pulse",
  );
  ...
  await createCategoria(
    ...
    "electro-cocina",
    "microwave",
    electrodomesticos.id_categoria,
  );
  await createCategoria(
    "Limpieza",
    "limpieza",
